  ...
  if (slidesBtns.length) {
    slidesBtns.forEach(function (slide, i) {
      slide.addEventListener('click', function () {
        slidesBtns.forEach(function (button) {
          button === slidesBtns[i] ? button.classList.toggle('active') : button.classList.remove('active');
        });
...
function slidesPlayer(state) {
  ...
  function playSlide() {
    var minimunToPlay = audio.dataset.duration * 0.9;
...
function slidesModeToggle(state) {
  ...
  var nextBtn = document.getElementById('next-btn');
  var prevBtn = document.getElementById('prev-btn');
  var hideInEditor = [expandBtn, slideImg, showNotesBtn, notesContent, nextBtn, prevBtn];
  var showInEditorFlex = [addSlideEl, addExistingFeaure, fileInputEl];
  var showInEditorBlock = [canvas, incanvasSlideName, incanvasSlideId, addNotesBtn, slideSubmitBtn, notesTextarea, notesSubmitBtn, recordBtn];
